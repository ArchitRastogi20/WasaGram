...
type AppDatabase interface {
	...
	// get photos for user profile
	GetPhotosUserProfile(User) (UserProfile, error)

	// get user stream

	GetUserStreamNew(User) (UserStream, error)

	// get all comments on a pic

	GetAllComments(PhotoId) ([]Comment_stream, error)

	// check if like exsits or not

	CheckIfLikeExists(User, PhotoId) (bool, error)
...
func createDatabase(db *sql.DB) error {
	// Define the database schema using SQL statements.
	createUserTable := `
...
`
	...
	CREATE TABLE IF NOT EXISTS Like (
		...
		ImageID INTEGER,
		Personwholikes INTEGER,
		Personwhoimageis INTEGER
	);
`
